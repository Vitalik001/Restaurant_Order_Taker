...
class GuestService:
    ...
    async def handle_message(session_id: int, message: str):
        ...
        elif match:=re.match(r"i don't want (an? )?(.+)$", message, re.IGNORECASE):
            item_name = match.group(2)
            item = await GuestUtils.check_item(item_name)
            if item:
        ...
        elif message == "That's all":
            order = await GuestUtils.get_order(session_id)
            return await GuestUtils.save_message(session_id, f"Your total is ${order['total_price']}. Thank you and have a nice day!")
