...
class GuestService:
    ...
    async def handle_message(session_id: int, message: str):
        ...
        if status == 1:
            ...
            await GuestUtils.save_message(session_id, message)
            return await GuestService.handle_state_6(session_id,message)
        elif status == 8:
            return await GuestService.handle_state_8()
    ...
    @staticmethod
    async def handle_state_6(session_id: int, message: str):
        if match := re.match(r"^i'd like (an? )?(.+?)(\.?)$", message, re.IGNORECASE):
            item_name = match.group(2)
            item = await GuestUtils.check_item(item_name)
            if item:
