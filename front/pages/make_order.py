...
with st.expander("Help"):
    ...
    st.markdown("- **I'd like a(an) X.**: This is said when the guest wants to order a specific item on the menu.")
    st.markdown("- **I don't want a(an) X.**: This is said when the guest changes their mind about a previously ordered item.")
    st.markdown("- **That's all.**: This is said after the guest has finished ordering.")
    st.markdown("- **Yes, please.**: This is said when the guest wants to accept the upsell.")
    st.markdown("- **No, thank you.**: This is said when the guest wants to reject the upsell.")
...
def clear_text():
    ...
    if re.match(r"^that's all(\.?)$", st.session_state["temp"], re.IGNORECASE):
